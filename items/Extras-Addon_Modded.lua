...
chatOpt:add_button("Send Message", function()
	...
	script.run_in_fiber(function(chatMsg)
        if isTeam:is_enabled() == false then
            if chatBox ~= "" then
                network.send_chat_message(""..chatBox, false) -- MODDED
            end
        else
            if chatBox ~= "" then
                network.send_chat_message(""..chatBox, true)  -- MODDED
            end
        end
        sleep(3)  -- MODDED
...
chatOpt:add_separator()
chatOpt:add_button("UwU", function()  -- MODDED
	if isCooldown then
    ...
    script.run_in_fiber(function(addonMsg)
        ...
        network.send_chat_message("UwU"..chatBox, false)  -- MODDED
        sleep(3) -- MODDED
        isCooldown = false  -- Reset the cooldown after the delay
    end)
end)
chatOpt:add_sameline()
chatOpt:add_button("Hello", function() -- MODDED
	if isCooldown then
    ...
    script.run_in_fiber(function(spawnClone)
        ...
        network.send_chat_message("Hello everybody! Have are beatifull gaming <3"..chatBox, false) --MODDED
        sleep(3)  -- MODDED
        isCooldown = false  -- Reset the cooldown after the delay
    end)
end)
